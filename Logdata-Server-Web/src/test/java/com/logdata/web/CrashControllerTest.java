...
public class CrashControllerTest {
    ...
    public void crashDataInputTest() {
        ...
        given(crashDataService.findOne("1")).willReturn(crashVO);
...
//                        )
    ...
    public void crashDataTimeTest() {
        try {
            given(userService.findByUserID("user")).willReturn(new UserVO("user", "user"));
            given(crashDataService.findCrashDataByTimeAndApiKeyAndPackageName(new DateTime(1L), "apiKey", "android")).willReturn(new CrashVO());
            // when
            MockHttpServletResponse response = mvc.perform(
